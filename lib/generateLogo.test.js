...
describe('generateLogo', () => {
    ...
    describe('makeLogo', () => {
        it('should return a complete svg file of all components', () => {
            ...
            const text = `<text x="145" y="125" dominant-baseline="middle" text-anchor="middle" fill="orange" font-size="64"> tri </text>`;
            let svg = triangle.render();
            const makeLogo = `${svgString} ${svg} ${text} </svg>`;
            expect(makeLogo).toEqual(`<svg version="1.1" width="300" height="200" xmlns="https://www.w3.org/2000/svg"> <polygon points="150, 0 0, 250 250, 200" fill="black" /> <text x="145" y="125" dominant-baseline="middle" text-anchor="middle" fill="orange" font-size="64"> tri </text> </svg>`)
        })
    })
})
